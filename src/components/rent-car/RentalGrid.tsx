"use client";

import { User, Snowflake, Zap, Settings, Fuel, Disc, Route, Star } from "lucide-react";
import { WHATSAPP_NUMBER } from "@/lib/constants";

const WhatsAppIcon = () => (
  <svg width="16" height="16" viewBox="0 0 448 512" fill="currentColor">
    <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5.1-3.9-10.6-6.9z" />
  </svg>
);

const specIcons: Record<string, React.ReactNode> = {
  user: <User size={14} />,
  snowflake: <Snowflake size={14} />,
  bolt: <Zap size={14} />,
  cogs: <Settings size={14} />,
  "gas-pump": <Fuel size={14} />,
  "compact-disc": <Disc size={14} />,
  road: <Route size={14} />,
  star: <Star size={14} />,
};

interface CarSpec {
  icon: string;
  label: string;
}

interface RentalCar {
  name: string;
  image: string;
  specs: CarSpec[];
  whatsappText: string;
}

const rentalCars: RentalCar[] = [
  {
    name: "Toyota Camry 2010",
    image: "/images/fleet-camry-muscle-2010.png",
    specs: [
      { icon: "user", label: "5" },
      { icon: "snowflake", label: "AC" },
      { icon: "cogs", label: "Auto" },
    ],
    whatsappText: "Hi, I'd like to rent the Toyota Camry 2010",
  },
  {
    name: "Honda Civic 2007",
    image: "/images/fleet-honda-civic-2007.png",
    specs: [
      { icon: "user", label: "5" },
      { icon: "snowflake", label: "AC" },
      { icon: "gas-pump", label: "Eco" },
    ],
    whatsappText: "Hi, I'd like to rent the Honda Civic 2007",
  },
  {
    name: "Peugeot 206 2005",
    image: "/images/fleet-peugeot-206-2005.png",
    specs: [
      { icon: "user", label: "5" },
      { icon: "bolt", label: "Manual" },
      { icon: "compact-disc", label: "Compact" },
    ],
    whatsappText: "Hi, I'd like to rent the Peugeot 206 2005",
  },
  {
    name: "Peugeot 406",
    image: "/images/fleet-peugeot-406.png",
    specs: [
      { icon: "user", label: "5" },
      { icon: "snowflake", label: "AC" },
      { icon: "road", label: "Comfort" },
    ],
    whatsappText: "Hi, I'd like to rent the Peugeot 406",
  },
  {
    name: "Toyota Corolla 2010",
    image: "/images/fleet-toyota-corolla-2010.png",
    specs: [
      { icon: "user", label: "5" },
      { icon: "snowflake", label: "AC" },
      { icon: "star", label: "Popular" },
    ],
    whatsappText: "Hi, I'd like to rent the Toyota Corolla 2010",
  },
  {
    name: "Bajaj Boxer 150",
    image: "/images/motocycle.png",
    specs: [
      { icon: "user", label: "2" },
      { icon: "bolt", label: "Manual" },
      { icon: "gas-pump", label: "Fuel Efficient" },
    ],
    whatsappText: "Hi, I'd like to rent the Bajaj Boxer 150",
  },
  {
    name: "Honda ACE 125",
    image: "/images/motocycle.png",
    specs: [
      { icon: "user", label: "2" },
      { icon: "bolt", label: "Manual" },
      { icon: "compact-disc", label: "Compact" },
    ],
    whatsappText: "Hi, I'd like to rent the Honda ACE 125",
  },
  {
    name: "TVS Apache 160",
    image: "/images/motocycle.png",
    specs: [
      { icon: "user", label: "2" },
      { icon: "bolt", label: "Manual" },
      { icon: "star", label: "Sport" },
    ],
    whatsappText: "Hi, I'd like to rent the TVS Apache 160",
  },
];

export default function RentalGrid() {
  return (
    <section className="py-15">
      <div className="max-w-[1180px] mx-auto px-5">
        <div className="grid grid-cols-[repeat(auto-fill,minmax(340px,1fr))] gap-7.5">
          {rentalCars.map((car) => (
            <div
              key={car.name}
              className="bg-surface rounded-xl overflow-hidden shadow-[0_4px_15px_rgba(0,0,0,0.08)] flex flex-col transition-transform duration-300 cursor-default hover:-translate-y-1.5"
            >
              <div className="w-full h-55 bg-surface-muted flex items-center justify-center overflow-hidden">
                <img src={car.image} alt={car.name} className="w-full h-full object-cover" />
              </div>
              <div className="p-6 flex flex-col flex-1">
                <div className="text-xl font-bold mb-2 text-dark">{car.name}</div>
                <div className="flex gap-4 mb-4 text-text-light text-sm">
                  {car.specs.map((spec, i) => (
                    <span key={i} className="flex items-center gap-1.5">
                      {specIcons[spec.icon]}
                      {spec.label}
                    </span>
                  ))}
                </div>
                <div className="text-primary font-extrabold text-xl mb-5">
                  Contact for Price
                </div>
                <a
                  href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(car.whatsappText)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 bg-accent-whatsapp text-surface p-3 rounded-lg font-semibold text-base no-underline transition-opacity duration-300 mt-auto hover:opacity-90"
                >
                  <WhatsAppIcon />
                  Rent Now
                </a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
